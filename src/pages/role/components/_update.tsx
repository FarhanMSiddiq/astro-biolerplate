...
const userSchema = z.object({
  nama: z
    .string()
    .min(1, { message: "Nama Role tidak boleh kosong" })
    .max(30, { message: "Maksimal 30 karakter" }),
});

const UpdateRole: React.FC<UpdateRoleProps> = ({ data }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [errZod, setErrZod] = useState<z.ZodError>();
  const [newNama, setNewNama] = useState(data.name);

  const handleCheckData = async () => {
    const check = await checkRole(data.id.toString());
    ...
    if (check.data.length > 0) {
      return Swal.fire({
        title: "Peringatan",
        text: "Role tidak dapat di edit karna sudah digunakan user",
        icon: "warning",
        confirmButtonText: "Oke",
      });
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[100]">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <h2 className="font-semibold text-lg flex items-center">
              <FaEdit size={20} className="mr-2" /> Edit Role
            </h2>
